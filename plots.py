...
def plot_anomalies(ds,var='T'):
    ...
    if res == 'season':
        ...
        plt.savefig(pathname+'plots/'+plot_title_path+'Monthly/'+str(current_year)+'-'+month_names[last_season_index]+'-'+var+'.png')
...
if __name__ == "__main__":
    # doing setup, reading inputs, etc.
    ...
    # use normals to calculate anomalies
    T_anom, P_anom = calc_anomalies(szn,norm)
    if region == 'ARCC':
        print("Total T anomaly for ARCC: ",mask_anomalies(T_anom,'T').values,"deg C")
        print("Total P anomaly for ARCC: ",mask_anomalies(P_anom,'P').values,"%")
